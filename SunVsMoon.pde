...
void draw()
{
  ...
  if (keyPressed)
  {
    ...
    if(key == '`')
    {
      ...
      killcount = 0;
      spawnrate = 120;
      field = new boolean[numLanes][numColumns];
      setup();
    }
  }
  ...
  if (menu == 0)
  {
    ...
    image(start, 0, 0);
    textFont(font2);
    text("Press space to continue" , 0, 700);
  }
  else if(menu == 5)
  {
    imageMode(CORNER);
    image(intro, 0, 0);
    text("Press space to continue", 400, 700);
  }
  if(menu >= 10)
  {
    imageMode(CENTER);
    ...
    if(gameover == true)
    {
      ...
      if(mememode == true) {
        ...
        //textFont(font);
        textFont(font);
        text("You failed to Nobulate (NOBNI) back to the manufacturing media (A:)\n           Press any key to restart", width/8, height/4 + 48*6);
      }
      else{
        imageMode(CORNER);
        image(loss, 0, 0);
      ...
      for(int i = 0; i < pew.size(); i++)
      {
        ...
        for(int j =  0; j < moonrock.size(); j++)
        {
          ...
          if((b.lane == m.lane) && (b.shootx >= m.x) && (b.shootx <= m.x + 180))
          {
            if(b.bullettype == 3) {
              m.speed = 0.5; 
              m.eatrate = 70; 
      ...
      for(int i = 0; i < moonrock.size(); i++)
      {
        ...
        for(int j = 0; j < sunlight.size(); j++)
        {
          ...
          else if((t.lane == m.lane) && (m.x <= t.barrelx) && (m.x > (t.column -1) * 180))
          {
            m.eating = true;
      ...
      if((millis() % 10000 >= 0) && (millis() % 10000 <= 10)) {
        fill(255);
        textFont(font3); 
        println("NEXT WAVE");
